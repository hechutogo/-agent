...
空爪确认需要新传感器证据：实际夹爪张开且物体已离开 TCP 邻域，或者物体
得到有效支撑且夹爪张开/已经分离。“已脱离”不表示“已放稳”；目标支撑关系
仍由独立的连续视觉检查确认。仅发送张爪命令不足以证明实际释放。
...
自研 Agent 与优化版共用 `scene/support.py` 中的桌面、堆叠和盒内关系判据，
以及 `scene/verification.py` 中的稳定采样器。目标关系必须连续两次成立；
单次验证最多采样三帧，相邻采样间在原仿真线程保持八个物理步。短暂遮挡、
物体尚未落稳不直接触发重新抓取或消耗一次任务重试。

桌面关系只使用实际可见底部，与桌面高度差要求 `-3 mm ≤ gap < 4 mm`；
盒内关系允许盒壁遮挡底部，但必须看到物体 footprint 在盒内、可见底部低于盒沿。
“盒内关系成立”与“夹爪已释放并由盒底支撑”仍为两个独立判据，不互相替代。

优化版在当前抓取执行开始、机械臂尚未下降时采集基线，基线只属于本次目标和
本次执行。采集失败不会留下已闭爪的候选；闭爪命令发出前才登记候选，
保证命令部分生效后异常仍保留状态。抓取确认依旧要求至少 3.5 cm 的视觉抬升。
历史 B 抓取图像中的两块蓝色碎片确由夹爪遮挡造成，因此保留歧义拒绝，
不会通过降低识别阈值或补洞制造抓取中心。

搬运到目的位置后重新测量物体与 TCP 的偏移，避免手腕转向后继续沿用源位置
标定。精确支撑面的优化版纠偏忽略 3 mm 以内的视觉噪声，容器则根据实测内壁
余量决定是否需要纠偏；纠偏后仍预检剩余完整路径。自研 `CarryTo` 更新偏移
供放置原子使用，测量失败就使旧偏移失效，禁止拿旧数据继续释放。

共享 `_cartesian` 在首个动作前预检整段插值路径，之后仍从实际关节状态逐点
求解和执行，TCP 跟踪容差仍为 8 mm。失败产物记录目标与实际 TCP、实际关节、
命令关节、误差和插值点编号。原子后置检查失败也保留已采集证据，便于 ReAct
和 Trace 解释原因。

初版 TiPToP 的规划、开环执行完成语义保持不变，但也使用增强后的共享运动预检；
对照测试时应区分“上层策略不变”和“底层执行实现完全相同”。
