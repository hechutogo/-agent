...
| 执行引擎 | 定位 | 核心特征 | 成功判定 |
| --- | --- | --- | --- |
| 自研 Agent | 默认主线 | 扁平原子计划、逐原子校验、通用 ReAct | 原始目标通过最终视觉复核 |
...
基础设施；感知算法可以按引擎组织。它们可以在同一场景、同一随机种子和同一
机器人约束下进行 A/B 对比。
...
优化版统一了 TAMP 和持物恢复使用的支撑规则：

- 物块中心在支撑边缘内至少 2 mm；
- 上层物块与支撑俯视 footprint 至少 50% 重叠；
- 物块底面与支撑顶面的允许间隙为 `-3～8 mm`；
- 盒内关系要求物块 footprint 完整位于盒内；
- 桌面接触优先使用实测底面；
- 只有盒壁遮挡物块底面时，才使用已知 36 mm 高度进行保守补偿。

叠放后重新抓取时，规划器会依次尝试 10、8、6 cm 抬升高度，并确保轨迹高于场景
障碍至少 3 cm。每个候选都执行连续路径 IK 预检。
...
任何阶段失败都返回结构化 `AtomResult`，而不是抛出未经分类的文本异常。
...
Reactor 返回公开的简短依据，不展示模型私有思维链。任何恢复动作仍需通过
`AtomRegistry` 参数和语义顺序校验。
...
执行器按源对象保存原始计划中最后一个 `verify_state`，它代表该对象的终局关系。
即使 Reactor 替换或重规划了中间动作，所有动作结束后仍会：

1. 重新读取 RGB-D；
2. 重新定位目标与支撑对象；
3. 重新计算物体关系；
4. 逐项执行原始 `verify_state`；
5. 全部通过后才返回成功。

模型文本、动作调用成功和旧 WorldState 都不能绕过这一终局复核。
...
运行中或持物状态不确定时禁止切换引擎，避免新引擎丢失物理上下文。
...
| 路径预检 | 8 mm 连续 IK | 关键点为主 | 与执行器一致的 8 mm 连续 IK |
...
| 最终成功 | 原始目标新鲜视觉复核 | 轨迹执行完成 | 每段目标视觉验证 |
